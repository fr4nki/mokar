...
const parseFile = (path, prefix) => {
    const LONGEST_LEN_METHOD_NAME_WITH_BRACKETS = 8;

    const rawFilename = path.toString().split('/').slice(-1)[0];

    const ok = !fileIsHidden(rawFilename);
    const { mime, filename: fileNameWithoutExtension, extension } = mapFileNametoMimeType(rawFilename);
    const { method, filename: filenameWithoutMethod } = getMethodFromString(fileNameWithoutExtension);
    const urlPath = prefix + '/' + filenameWithoutMethod;
        ...
        files.forEach((file) => {
            ...
            if (checkPermissionToRead(absFilePath)) {
                if (fileIsDirectory) {
                    ...
                    const file = parseFile(absFilePath, prefix);
                    file.ok && tree.push(file);
                }
            }
        })
    } else {
        const file = parseFile(dir, prefix);
        file.ok && tree.push(file);
    }
...
const makeTree = (config) => {
    ...
    return config.mocks.map(({ from, prefix }) => {
        ...
        return readTree(mockPath, prefix, []);
    });
};
...
const action = (config) => () => {
    ...
    http
        .createServer({}, (req, res) => {
            ...
            setTimeout(() => {
                const founded = tree
                    .filter(item => item.method === method.toLowerCase() || item.method === 'all')
                    .filter(item => url.match(new RegExp(item.matcher, 'g')));
